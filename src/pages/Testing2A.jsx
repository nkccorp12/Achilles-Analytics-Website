...
import './Testing2A.css';
import './LandingPage.css';

/* ======================================================================
   TESTING 2A — PIPELINE FLOW
   One unified left-to-right data-flow visualization across all 6 modules.
   Raw signal enters left, finished intel emerges right.
   ====================================================================== */
...
const STAGES = [
  {
    ...
    group: 'EYES',
    name: 'CRISIS MAP',
    caption: 'Geo-spatial incident detection.',
    shape: 'dots',
  },
  {
    id: 2,
    group: 'EYES',
    name: 'NEWS HUB',
    caption: 'Live OSINT ingestion stream.',
    shape: 'feed',
  },
  {
    id: 3,
    group: 'BRAIN',
    name: 'DIGITAL TWIN',
    caption: 'Adversary network graph.',
    shape: 'graph',
  },
  {
    id: 4,
    group: 'BRAIN',
    name: 'BIAS CONTROL',
    caption: 'Detection wheel calibration.',
    shape: 'wheel',
  },
  {
    id: 5,
    group: 'VOICE',
    name: 'AI COUNCIL',
    caption: 'Multi-persona LLM debate.',
    shape: 'council',
  },
  {
    id: 6,
...
/* ----------- Stage Glyphs (the "data shape" at each step) ----------- */
...
/* ----------- Connector between two stages (animated dashed line) ----------- */
...
const Connector = ({ fromColor, toColor, index }) => (
  <svg viewBox="0 0 100 60" className="t2a-connector" preserveAspectRatio="none">
    ...
    {/* Static rail */}
    <line x1="0" y1="30" x2="100" y2="30" stroke={`url(#grad-${index})`} strokeWidth="0.6" opacity="0.25" />
    {/* Animated packet trail */}
    <line x1="0" y1="30" x2="100" y2="30" stroke={`url(#grad-${index})`} strokeWidth="1" strokeDasharray="3 5" opacity="0.9">
...
const Testing2A = () => {
  ...
  // Sequentially highlight stages along the pipeline
  useEffect(() => {
  ...
  return (
    <div className="t2a">
      {/* Fixed header, reusing LandingPage.css */}
      <header className={`neu-header ${headerVisible ? 'neu-header--visible' : ''}`}>
        <div className="neu-header__inner">
          <a href="/" className="neu-header__wordmark">
            ACHILLES<span>.</span>
            <span className="neu-header__wordmark-sub">ANALYTICS</span>
          </a>
      ...
      <main className="t2a-main">
        {/* Top label + heading */}
        <section className="t2a-intro">
          <div className="t2a-eyebrow">
            <span className="t2a-eyebrow-bracket">[</span>
            <span className="t2a-eyebrow-text">// PIPELINE FLOW</span>
            <span className="t2a-eyebrow-dot" />
            <span className="t2a-eyebrow-meta">VARIANT.A — LINEAR.HORIZONTAL</span>
            <span className="t2a-eyebrow-bracket">]</span>
          </div>
          <h1 className="t2a-headline">From signal to decision.</h1>
          <p className="t2a-sub">
            Six modules. One continuous pipeline. Raw signal enters the left rail, gets
            shaped, scored, debated, and emerges as a finished intelligence brief.
          </p>
        </section>

        {/* Pipeline status strip */}
        <section className="t2a-strip">
          <div className="t2a-strip-cell">
            <span className="t2a-strip-key">INGRESS</span>
            <span className="t2a-strip-val t2a-strip-val--live">
              <span className="t2a-pulse" /> LIVE
            </span>
          </div>
          <div className="t2a-strip-cell">
            <span className="t2a-strip-key">STAGES</span>
            <span className="t2a-strip-val">06 / 06</span>
          </div>
          <div className="t2a-strip-cell">
            <span className="t2a-strip-key">LATENCY</span>
            <span className="t2a-strip-val">~ 4.2 s</span>
          </div>
          <div className="t2a-strip-cell">
            <span className="t2a-strip-key">EGRESS</span>
            <span className="t2a-strip-val">PDF.BRIEF</span>
          </div>
        </section>

        {/* THE PIPELINE */}
        <section className="t2a-pipeline" aria-label="Achilles pipeline flow">
          {/* Continuous bottom rail (the unifying spine) */}
            ...
            {STAGES.map((stage, i) => {
              ...
              return (
                <React.Fragment key={stage.id}>
                  <div
                    className={`t2a-stage t2a-stage--${stage.group.toLowerCase()} ${
                      isActive ? 'is-active' : ''
                    }`}
                  ...
                  {i < STAGES.length - 1 && (
                    <div className="t2a-connector-wrap">
                      ...
                      <span className="t2a-connector-label">
                        {STAGES[i].group !== STAGES[i + 1].group
                          ? `→ ${STAGES[i + 1].group}`
                          : '→'}
                      </span>
                    </div>
          ...
          <div className="t2a-terminal t2a-terminal--out">
            ...
            <div className="t2a-terminal-caption">INTEL.BRIEF</div>
          </div>
        </section>
        ...
        <footer className="t2a-footer">
          <div className="t2a-footer-row">
            <span className="t2a-footer-key">// END.OF.PIPELINE</span>
            <a href="/testing2" className="t2a-footer-link">
              ← RETURN TO TESTING2
            </a>
          </div>
        </footer>
